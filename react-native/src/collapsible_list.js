...
export default class CollapsibleList extends Component {
  ...
  toggle () {
    ...
    const { onToggle, collapsedHeight } = this.props
    let nextHeight

    if (collapsed) {
      nextHeight = minHeight + collapsedHeight
    } else {
      ...
      <View style={wrapperStyle}>
        {
          ...
          <View zIndex={2}>
            <TouchableOpacity onPress={() => this.toggle()} activeOpacity={0.5}>
              {buttonContent}
            </TouchableOpacity>
          </View>
        }
        <Animated.View style={{ overflow: 'hidden', height: animation, zIndex:1 }}>
