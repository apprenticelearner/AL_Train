...
import {baseName, shuffleArray, sampleArray} from "./utils"
...
async function gen_pretraining(args,context,shuffle=false){
  ...
  if(shuffle){problem_pool = shuffleArray(problem_pool)}
  var out = []
...
async function mirror_students(args,context){
  ...
  for(var p of human_problems){
    human_path_map[baseName(p).split(".")[0]] = p
  } 
